...
Usage:
    python -m evals.run_evals [--type TYPE] [--agent AGENT] [--persist]

Runs all eval suites (or filtered by type/agent) and optionally persists
results to the eval_results table in PostgreSQL.
"""
...
from evals.test_tool_correctness import deterministic_tool_check
...
async def run_all(persist: bool = False) -> dict:
...
def main():
    ...
    parser.add_argument("--type", choices=["hallucination", "tool_correctness", "trajectory", "all"],
                        default="all", help="Eval type to run")
    parser.add_argument("--persist", action="store_true", help="Persist results to DB and file")
    args = parser.parse_args()

    async def _run():
        if args.type == "all":
            results = await run_all(persist=args.persist)
        elif args.type == "hallucination":
            suite = await run_grounding_suite()
            results = suite.summary()
        elif args.type == "tool_correctness":
            suite = await run_tool_correctness_suite()
            results = suite.summary()
        elif args.type == "trajectory":
            suite = await run_trajectory_suite()
            results = suite.summary()
        else:
            results = await run_all(persist=args.persist)
